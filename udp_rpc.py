...
class UDP_RPC(protocol.DatagramProtocol):
  msgSizeLimit = udpDatagramMaxSize-26
  ...
  def sendRPC(self, contact, method, args, rawResponse=False):
    ...
    """
    msg = msgtypes.RequestMessage(self._node.ring_id, self._node.id, method, args)
    ...
    timeoutCall = reactor.callLater(rpcTimeout, self._msgTimeout, msg.id)
    # Transmit the data
    #print 'sending message to %s:%s' % (contact.address, contact.port)
  ...
  def datagramReceived(self, datagram, address):
    ...
    remoteContact = Contact(ring_id=message.ringID, id=message.nodeID, ip=address[0], port=address[1], network_protocol=self)
    #print 'received datagram from remote node %s:%s' % (address[0], address[1])        

    # Refresh the remote node's details in the local node's k-buckets
    self._node.add_contact(remoteContact)
  ...
  def _sendResponse(self, contact, rpcID, response):
    ...
    """
    msg = msgtypes.ResponseMessage(rpcID, self._node.ring_id, self._node.id, response)
    msgPrimitive = self._translator.toPrimitive(msg)
    encodedMsg = self._encoder.encode(msgPrimitive)
  ...
  def _sendError(self, contact, rpcID, exceptionType, exceptionMessage):
    ...
    """
    msg = msgtypes.ErrorMessage(rpcID, self._node.ring_id, self._node.id, exceptionType, exceptionMessage)
    msgPrimitive = self._translator.toPrimitive(msg)
    encodedMsg = self._encoder.encode(msgPrimitive)
    self._send(encodedMsg, rpcID, (contact.address, contact.port))
  ...
  def _msgTimeout(self, messageID):
    ...
    if self._sentMessages.has_key(messageID):
      ...
      if self._partialMessages.has_key(messageID):
        ...
        timeoutCall = reactor.callLater(rpcTimeout, self._msgTimeout, messageID) #IGNORE:E1101
      ...
      self._node.remove_contact(remoteContactID)
      df.errback(failure.Failure(TimeoutError(remoteContactID)))
    else:
